...
wavelet_dict = {
    'haar' : { 'g' : np.array([0.7071067811865475, 0.7071067811865475]) },

    'd4'   : { 'g' : np.array([0.4829629131445341, 0.8365163037378077,
                               0.2241438680420134, -0.1294095225512603]) }, 

    'mb4'  : { 'g' : np.array([4.801755e-01, 8.372545e-01, 2.269312e-01, -1.301477e-01]) },

    # TODO: Handle g, h, gd, hd
    'bs3.1': { 'g' : np.array([0.1767767, 0.5303301, 0.5303301, 0.1767767]),
               'gd': np.array([0.3535534, 1.06066, -1.06066, -0.3535534]) },

    'w4'   : { 'g' : np.array([-1, 3, 3, -1]) / 8.0,
               'h' : np.array([-1, 3, -3, 1]) / 8.0 },
    
    'fk4': { 'g' : np.array([.6539275555697651, .7532724928394872,
                             .5317922877905981e-1, -.4616571481521770e-1]) },
    
    'd6': { 'g' : np.array([0.3326705529500827, 0.8068915093110928, 0.4598775021184915,
                            -0.1350110200102546, -0.0854412738820267, 0.0352262918857096]) },
    
    'fk6': { 'g' : np.array([.4279150324223103, .8129196431369074, .3563695110701871,
                             -.1464386812725773, -.7717775740697006e-1, .4062581442323794e-1]) },
    
    'd8': { 'g' : np.array([0.2303778133074431, 0.7148465705484058, 0.6308807679358788,
                            -0.0279837694166834, -0.1870348117179132, 0.0308413818353661,
                            0.0328830116666778, -0.0105974017850021]) },
    
    'fk8': { 'g' : np.array([.3492381118637999, .7826836203840648, .4752651350794712,
                             -.9968332845057319e-1, -.1599780974340301, .4310666810651625e-1,
                             .4258163167758178e-1, -.1900017885373592e-1]) },

    'la8': { 'g' : np.array([-0.07576571478935668, -0.02963552764596039, 0.49761866763256290, 
                             0.80373875180538600, 0.29785779560560505, -0.09921954357695636, 
                             -0.01260396726226383, 0.03222310060407815]) },
    
    'mb8': { 'g' : np.array([-1.673619e-01, 1.847751e-02, 5.725771e-01, 7.351331e-01,
                             2.947855e-01, -1.108673e-01, 7.106015e-03, 6.436345e-02])[::-1] },  # Reversed

    'bl14': { 'g' : np.array([0.0120154192834842, 0.0172133762994439, -0.0649080035533744,
    ...
    'fk14': { 'g' : np.array([.2603717692913964, .6868914772395985, .6115546539595115,
    ...
    'd16': { 'g' : np.array([0.0544158422431049, 0.3128715909143031, 0.6756307362972904,
    ...
    'la16': { 'g' : np.array([-0.0033824159513594, -0.0005421323316355, 0.0316950878103452, 
    ...
    'mb16': { 'g' : np.array([-1.302770e-02, 2.173677e-02, 1.136116e-01, -5.776570e-02, 
                              -2.278359e-01, 1.188725e-01, 6.349228e-01, 6.701646e-01, 
                              2.345342e-01, -5.656657e-02, -1.987986e-02, 5.474628e-02, 
                              -2.483876e-02, -4.984698e-02, 9.620427e-03, 5.765899e-03])[::-1] },  # reversed

    'la20': { 'g' : np.array([0.0007701598091030, 0.0000956326707837, -0.0086412992759401,
    ...
    'bl20': { 'g' : np.array([0.0008625782242896, 0.0007154205305517, -0.0070567640909701,
    ...
    'fk22': { 'g' : np.array([.1938961077599566, .5894521909294277, .6700849629420265,
    ...
    'mb24': { 'g' : np.array([-2.132706e-05, 4.745736e-04, 7.456041e-04, -4.879053e-03,
                              ...
                              -1.644859e-02, -2.062335e-03, 1.193006e-03, 5.361301e-05])[::-1] }  # reversed
...
def wavelet_filter(name):
    ...
    The 'name' is one of: [ 'haar','d4','mb4','w4','bs3.1','fk4','d6','fk6','d8','fk8','la8',
                            'mb8','bl14','fk14','d16','la16','mb16','la20','bl20','fk22','mb24']
    '''
